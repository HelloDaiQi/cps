...
<script>
    ...
    window.operateEvents = {
        'click .RoleOfedit': function (e, value, row, index) {
            ...
            $.ajax({
                type: "post",
                url: "${cps}/message/updateMessageStatus.do",
        ...
        'click .RoleOfreply': function (e, value, row, index) {
            var sender=row.sender;
            var content='<form action="${cps}/message/replyMessage.do" method="post">\n' +
                '\t\t\t\t\t<input type="hidden" name="receiver" value="'+sender+'">\n' +
                '\t\t\t\t\t<div class="form-group">\n' +
                '\t\t\t\t\t\t<label for="name">回复</label>\n' +
                '\t\t\t\t\t\t<textarea name="content" class="form-control" rows="9" cols="25"\n' +
                '\t\t\t\t\t\t\trequired="required" placeholder="请输入回复内容"></textarea>\n' +
                '\t\t\t\t\t</div>\n' +
    ...
    function operateFormatter(value, row, index) {
        return [
            ...
            '<input type="button" value="回复" class="RoleOfreply btn btn-success btn-sm"  data-toggle="modal" style="display:inline">' +
...
<script type="text/javascript">
    ...
    $("#messageTable").bootstrapTable({
        url:'${cps}/message/messageList.do?type=receive',
        //是否显示查找
        search: false,
        //是否分页
        pagination: true,
